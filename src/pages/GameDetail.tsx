  ...
  const fetchGame = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('games').select('*').eq('id', id).maybeSingle();
  ...
  const checkWishlist = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('wishlist').select('id').eq('user_id', user.id).eq('game_id', id).maybeSingle();
      if (error) throw error;
  ...
  const toggleWishlist = async () => {
    ...
    try {
      if (isInWishlist) {
        const {
          error
        } = await supabase.from('wishlist').delete().eq('user_id', user.id).eq('game_id', id);
        if (error) throw error;
  ...
  return <div className="min-h-screen">
      ...
      <div className="pt-24 pb-20">
        ...
        <section className="relative h-[60vh] mb-12">
          ...
          backgroundImage: `url(${game.hero_image_url || game.image_url})`
        }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
              ...
              delay: 0.2
                ...
                {game.features && game.features.length > 0 && <>
                    ...
                    <ul className="space-y-3 mb-8">
                      {game.features.map((feature, index) => <motion.li key={index} initial={{
                    opacity: 0,
                    x: -20
                  }} animate={{
                    opacity: 1,
                ...
                {game.screenshots && game.screenshots.length > 0 && <>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {game.screenshots.map((screenshot, index) => <motion.img key={index} initial={{
                    opacity: 0,
                    scale: 0.9
                  }} animate={{
                    opacity: 1,
                    ...
                    delay: 0.4 + index * 0.1
                  }} src={screenshot} alt={`Screenshot ${index + 1}`} className="rounded-lg w-full hover-lift" />)}
                    </div>
                  </>}
              </motion.div>
            </div>
            ...
            <div className="lg:col-span-1">
              ...
              delay: 0.3
                ...
                <Button size="lg" className="w-full bg-accent text-accent-foreground hover:bg-accent/90 glow-gold mb-3" onClick={handlePurchase}>
                  <ShoppingCart className="mr-2" size={20} />
                  Buy Now
                </Button>

                <Button size="lg" variant="outline" className={`w-full border-accent hover:bg-accent/10 ${isInWishlist ? 'text-red-500' : 'text-foreground'}`} onClick={toggleWishlist}>
